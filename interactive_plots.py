...
def plot_data(df, base_path, plot_name,mode):
    ...
    plot(fig, filename=os.join(base_path, plot_name + ".html"))  # make, and save plot
    df = df.drop(["new_x", "new_y", "sizes"], axis=1)
    df.to_csv(os.join(base_path, plot_name + "_plot.csv"))
...
mode="P"
plot_name = "plots\\"+mode+"_plot_top_"+str(n_entries*n_entries)
base_path = "predictions"
df_map = pd.read_csv(os.join(base_path, mode+"_deduped.csv"))#for the clustering, so that all deduped entities are lustered and plotted next to the "main"entity
df_data = pd.read_csv(os.join(base_path, "predictionsLENA_"+mode+".csv"))
